...
SRC := src/main.c src/util.c src/sha256.c src/lexer.c src/parser.c \
       ...
       src/device.c src/qbc.c src/vm.c
...
DEVICE_GPIO_TEST := build/test_device_gpio
...
$(U32_SCALAR_TEST): tests/test_u32_scalar.c \
                   src/qbc.c src/qir.c src/guard.c src/util.c | build
	$(CC) $(CPPFLAGS) $(CFLAGS) \
		tests/test_u32_scalar.c \
		src/qbc.c src/qir.c src/guard.c src/util.c \
		-o $@ $(LDLIBS)
...
$(U32_COMPARISON_TEST): tests/test_u32_comparisons.c \
                       src/qbc.c src/qir.c src/guard.c src/util.c | build
	$(CC) $(CPPFLAGS) $(CFLAGS) \
		tests/test_u32_comparisons.c \
		src/qbc.c src/qir.c src/guard.c src/util.c \
		-o $@ $(LDLIBS)

$(IF_ELSE_TEST): tests/test_if_else_control_flow.c \
                 src/qbc.c src/qir.c src/guard.c src/util.c | build
	$(CC) $(CPPFLAGS) $(CFLAGS) \
		tests/test_if_else_control_flow.c \
		src/qbc.c src/qir.c src/guard.c src/util.c \
		-o $@ $(LDLIBS)

$(BOUNDED_REPEAT_TEST): tests/test_bounded_repeat.c \
                       src/qbc.c src/qir.c src/guard.c src/util.c | build
	$(CC) $(CPPFLAGS) $(CFLAGS) \
		tests/test_bounded_repeat.c \
		src/qbc.c src/qir.c src/guard.c src/util.c \
		-o $@ $(LDLIBS)
...
$(DEVICE_GPIO_TEST): tests/test_device_gpio.c \
                    src/device.c src/qbc.c src/qir.c src/guard.c src/util.c | build
	$(CC) $(CPPFLAGS) $(CFLAGS) \
		tests/test_device_gpio.c \
		src/device.c src/qbc.c src/qir.c src/guard.c src/util.c \
		-o $@ $(LDLIBS)
	chmod 0755 $@
